...
func (p *btEpool) get(err error, hit bool, i int, k string, q *d, t *Tree, ver int64) *Enumerator {
	x := p.Get().(*Enumerator)
...
func (t *Tree) extract(q *d, i int) { // (r *manifest.Content) {
	t.ver++
...
func (t *Tree) First() (k string, v *manifest.Content) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k string) (v *manifest.Content, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k string, v *manifest.Content) *d {
	t.ver++
	q.setTree(t)
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k string, v *manifest.Content) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k string, v *manifest.Content) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k string, v *manifest.Content) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
//
// 	tree.Put(k, func(string, bool){ return v, true })
//
// modulo the differing return values.
func (t *Tree) Put(k string, upd func(oldV *manifest.Content, exists bool) (newV *manifest.Content, write bool)) (oldV *manifest.Content, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV *manifest.Content
	if q == nil {
...
func (t *Tree) split(p *x, q *d, pi, i int, k string, v *manifest.Content) {
	t.ver++
	r := btDPool.Get().(*d)
	r.setTree(t)
	if q.n != nil {
...
// io.EOF is returned.
func (e *Enumerator) Next() (k string, v *manifest.Content, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k string, v *manifest.Content, err error) {
	if err = e.err; err != nil {
		return
	}
